...
#pragma endregion
		...
		if (!RejectPole() && InspectionActive(InspectionNo))
		{
			...
			float emptySize, minSize, maxSize, maxAngle;
			HTuple sliderType;
			...
			for (int i = 0; i < roiIndex.Num(); i++)
			{
				...
				//格式 "L/R; emptySize, 20; minSize, 15.3; maxSize, 19.8; maxAngle, 5;"
				...
				for (int k = 0; k < keyCnt; k++)
				{
					...
					switch (k)
					{
					   ...
					   case 1: emptySize = stof(seg[1].S()); break;
					   case 2: minSize = stof(seg[1].S()); break;
					   case 3: maxSize = stof(seg[1].S()); break;
					   case 4: maxAngle = stof(seg[1].S()); break;
					}
				}
					...
					else
					{
						...
						if (!poSMs->Err(ShapeModelActive))
						{
							...
							HRegion roiSilder;
							HTuple emptyType = HTuple(parts[1]).Split(",");
							
							/*double maxEmpty;*/
							/*switch (ArticleParameter(1))
							{
							case ArticleColor_BU:maxEmpty = partsAttribute.slider_emptySize[partsAttribute.BU]; break;
							case ArticleColor_GY:maxEmpty = partsAttribute.slider_emptySize[partsAttribute.GY]; break;
							case ArticleColor_GNYE:maxEmpty = partsAttribute.slider_emptySize[partsAttribute.GNYE]; break;
							case ArticleColor_WH:maxEmpty = partsAttribute.slider_emptySize[partsAttribute.WH]; break;
							case ArticleColor_RD:maxEmpty = partsAttribute.slider_emptySize[partsAttribute.RD]; break;
							case ArticleColor_VT:maxEmpty = partsAttribute.slider_emptySize[partsAttribute.VT]; break;
							case ArticleColor_BN:maxEmpty = partsAttribute.slider_emptySize[partsAttribute.BN]; break;
							case ArticleColor_YE:maxEmpty = partsAttribute.slider_emptySize[partsAttribute.YE]; break;
							case ArticleColor_OG:maxEmpty = partsAttribute.slider_emptySize[partsAttribute.OG]; break;
							case ArticleColor_BK:maxEmpty = partsAttribute.slider_emptySize[partsAttribute.BK]; break;
							}*/
							...
							if (sliderDeviation > emptySize)
							{
								snprintf(str_de, sizeof(str_de), "Sliderfläche: %.2f  ( Max %.2f)", sliderDeviation, emptySize);
								snprintf(str_en, sizeof(str_en), "Slider burr: %.2f  (M Max %.2f)", sliderDeviation, emptySize);
								SelectLanguage(RejectMessage, str_de, str_en);
					...
					if (ArticleParameter(1) != ArticleColor_OG)
					{
						...
						MinValue = minSize/*partsAttribute.slider_minSize[0]*/;
						MaxValue = maxSize/*partsAttribute.slider_maxSize[0]*/;
					}
					else
					{						
						...
						MinValue = minSize/* partsAttribute.slider_minSize[4]*/;
						MaxValue = maxSize /*partsAttribute.slider_maxSize[4]*/;
					}				
