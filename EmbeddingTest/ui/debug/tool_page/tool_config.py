...
def _group_name_options_for_camera(tool_page, camera_role: object) -> list[str]:
    ...
    for inspection_item in getattr(tool_page, "inspection_items", []) or []:
        ...
        if item_role != role:
            continue
        task_group = str(getattr(inspection_item, "task_group", "") or "").strip()
        if not task_group or task_group in seen:
            continue
        options.append(task_group)
        seen.add(task_group)
...
def _inspection_item_status(tool_page, inspection_item):
    ...
    is_anomaly_tool = getattr(tool_page.algo, "is_anomaly_tool", lambda _code: False)
    if is_anomaly_tool(inspection_item.algorithm_code):
        algorithm = tool_page.algo.resolve_tool_algorithm(inspection_item.algorithm_code)
        anomaly_model_path = getattr(tool_page.algo, "anomaly_model_path", None)
        if callable(anomaly_model_path):
            ...
            model_path = os.path.join(tool_page.session.product_dir, f"anomaly_model_{algorithm}.npz")
    ...
    tooltip = (
        f"传统工具未标定\n"
        f"算法: {tool_page.algo.algorithm_display_name(algorithm) or algorithm}\n"
        f"存储键: {tool_page.algo.traditional_model_storage_key(algorithm, model_key=inspection_item.effective_model_key)}"
    )
...
def _refresh_inspection_items_table(tool_page) -> None:
    ...
    try:
        ...
        for row, actual_index in enumerate(visible_indexes):
            ...
            group_combo = QtWidgets.QComboBox(table)
            ...
            group_combo.currentIndexChanged.connect(
                lambda _index, row_index=actual_index, combo=group_combo: tool_page._on_inspection_item_group_changed(
                    row_index,
                    combo.currentData(),
                )
            )
            table.setCellWidget(row, 4, group_combo)
...
def _on_inspection_items_selection_changed(tool_page) -> None:
    ...
    item = _selected_inspection_item(tool_page)
    if item is None:
        apply_runtime_params = getattr(tool_page, "_apply_runtime_params_to_ui", None)
        if callable(apply_runtime_params):
    ...
    sync_item_runtime_params_to_controller(tool_page, item, algorithm=algorithm)
    apply_runtime_params = getattr(tool_page, "_apply_runtime_params_to_ui", None)
    if callable(apply_runtime_params):
        apply_runtime_params()
    tool_page._refresh_lists()
...
def _on_inspection_item_group_changed(tool_page, row: int, group_name: object) -> None:
    if getattr(tool_page, "_inspection_items_table_loading", False):
        return
    if row < 0 or row >= len(tool_page.inspection_items):
        return
    normalized = str(group_name or "").strip()
    tool_page.inspection_items[row].task_group = normalized
    _persist_inspection_items(tool_page)
    _refresh_inspection_items_table(tool_page)
...
__all__ = [
    ...
    "_on_inspection_item_group_changed",
