...
def build_menu_bar(window) -> None:
    ...
    algo_menu.addAction(
        shell_icon(SP.SP_FileDialogContentsView), "打开异常热力图"
    ).triggered.connect(window.tool_page.open_anomaly_heatmap_tool)
    algo_menu.addAction(
    ...
    runtime_menu.addAction(
        shell_icon(SP.SP_BrowserStop), "\u89e6\u53d1\u65f6\u5e8f\u8bbe\u7f6e..."
    ).triggered.connect(window._show_trigger_timing_settings_dialog)
