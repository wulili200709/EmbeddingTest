...
def normalize_foot_trigger_delay_ms(value: object) -> int:
    try:
...
def load_foot_trigger_delay_from_session(window) -> int:
    session_data = window.session.load_session()
    return normalize_foot_trigger_delay_ms(session_data.foot_trigger_delay_ms)
...
def apply_foot_trigger_delay(
    ...
    *,
    ...
    normalized = normalize_foot_trigger_delay_ms(delay_ms)
    window._foot_trigger_delay_ms = normalized
    if hasattr(window, "runtime_ctrl") and window.runtime_ctrl is not None:
        window.runtime_ctrl.update_foot_trigger_delay_ms(normalized)
    if persist:
        persist_foot_trigger_delay(window, normalized)
    if show_message:
        window._bottom_status_bar.showMessage(
            f"脚踏触发延时已更新：{normalized} ms",
            3000,
        )
