...
class _ToolPageRuntimeWidgetsHarness:
    ...
    def _is_anomaly_algorithm(self, algorithm: str | None = None) -> bool:
...
class ToolPageRuntimeWidgetsTest(unittest.TestCase):
    ...
    def test_anomaly_tool_forces_topk_mode(self) -> None:
        harness = _ToolPageRuntimeWidgetsHarness()
        harness.anomaly = True
        harness.cmb_mode.setCurrentText("proto")

        harness._update_runtime_widgets()

        self.assertEqual(harness.cmb_mode.currentText(), "topk")
        self.assertFalse(harness.cmb_mode.isEnabled())
        self.assertTrue(harness.spin_topk.isEnabled())
        self.assertFalse(harness.btn_embedding_analysis.isEnabled())
        self.assertTrue(harness.btn_validate_margin.isEnabled())
