...
class _FakeAnomalyInspectionExecutor:
    last_request = None
    ...
    def execute(self, request):
        ...
        return SimpleNamespace(
            ...
            raw_row={
                "item_rows": [
                    {"pred": "OK", "diff": 0.0123, "value": 0.0877, "threshold": 0.1000},
                    {"pred": "NG", "diff": -0.2200},
                ]
            },
            item_results=[
...
class ToolPageRunTestAllItemsTest(unittest.TestCase):
    ...
    def test_run_test_status_includes_anomaly_score_explanation(self) -> None:
        with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as handle:
            image_path = handle.name
        ...
        original_executor = page_module.InspectionExecutor
        try:
            page_module.InspectionExecutor = _FakeAnomalyInspectionExecutor
            harness._run_test()
        finally:
            page_module.InspectionExecutor = original_executor
            os.unlink(image_path)

        self.assertIn("anomaly=", harness.lbl_status.text)
        self.assertIn("ROI1:0.0877<=0.1000", harness.lbl_status.text)
        self.assertIn("gap=0.0123", harness.lbl_status.text)
