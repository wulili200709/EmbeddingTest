...
class _SampleAnnotationHarness:
    ...
    _mark_sample_path_all_ok = ToolPage._mark_sample_path_all_ok
    _mark_sample_path_all_ng = ToolPage._mark_sample_path_all_ng
    ...
    def __init__(self, product_dir: str) -> None:
        ...
        self.algo = SimpleNamespace(tool_model_key=lambda value: str(value or "").strip())
...
class ToolPageSampleAnnotationsTest(unittest.TestCase):
    ...
    def test_sample_roi_status_persists_to_json_store(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(reloaded._sample_annotation_state_for_path(str(image_path), "cam1"))
    ...
    def test_tool_stats_and_validation_use_roi_annotations(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("OK 1 / NG 0", harness._current_tool_sample_stats_text())
            self.assertIn("还有 1 张图片未标注", harness._training_validation_text())


    def test_mark_sample_path_all_ng_marks_all_present_roi_as_ng(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            image_path = Path(tmpdir) / "cam1_sample.png"
            image_path.write_bytes(b"img")
            self._write_labelme_json(image_path, ["roi1", "roi2"])
