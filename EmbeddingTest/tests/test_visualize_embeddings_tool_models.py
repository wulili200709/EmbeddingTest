...
class VisualizeEmbeddingsToolModelsTest(unittest.TestCase):
    ...
    def test_load_product_analysis_group_proto_model_uses_proto_points(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            session_root = Path(tmpdir)
            product_dir = session_root / "demo_product"
            product_dir.mkdir(parents=True, exist_ok=True)
        ...
        self.assertEqual(result.point_names, ["OK proto", "NG proto"])
        self.assertEqual([row.file_name for row in result.rows], ["OK proto", "NG proto"])
