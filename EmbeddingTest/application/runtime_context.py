    ...
    for algorithm, group in learning_groups.items():
        ...
        for item, model, embedding in zip(group, models, embeddings):
            ...
            rows_by_key[item.model_key] = {
                ...
                "value": None,
                "threshold": None,
                "margin": float(getattr(model, "margin", getattr(algo.product_params, "margin", 0.0))),
                "score_mode": str(
                    getattr(model, "score_mode", getattr(algo.product_params, "score_mode", "proto")) or "proto"
                ),
                "topk": int(getattr(model, "topk", getattr(algo.product_params, "topk", 1))),
                "match_ms": match_ms,
                "infer_ms": 0.0,
                "total_ms": 0.0,
                "json_name": json_name,
    ...
    *,
    ...
    for algorithm, group in learning_groups.items():
        ...
        for item, model, embedding in zip(group, models, embeddings):
            ...
            rows_by_key[item.model_key]["margin"] = float(
                getattr(model, "margin", getattr(algo.product_params, "margin", 0.0))
            )
            rows_by_key[item.model_key]["score_mode"] = str(
                getattr(model, "score_mode", getattr(algo.product_params, "score_mode", "proto")) or "proto"
            )
            rows_by_key[item.model_key]["topk"] = int(
                getattr(model, "topk", getattr(algo.product_params, "topk", 1))
            )
        group_infer_total_ms = float((time.perf_counter() - group_infer_t0) * 1000.0)
        per_item_infer_ms = group_infer_total_ms / float(len(group)) if group else 0.0
        for item in group:
