...
from .anomaly import ANOMALY_ALGORITHMS, PATCHCORE_LITE_ALGORITHM
...
_ANOMALY_DISPLAY_NAMES = {
    PATCHCORE_LITE_ALGORITHM: "PatchCore Lite 异常检测",
}
...
for _code in ANOMALY_ALGORITHMS:
    _TOOL_ALGORITHM_SPECS[_code] = ToolAlgorithmSpec(
        code=_code,
        display_name=_ANOMALY_DISPLAY_NAMES.get(_code, _code),
        family="anomaly",
        fit_mode="anomaly",
        default_params={},
    )
...
def list_tool_algorithm_specs() -> List[ToolAlgorithmSpec]:
    return [
        ...
        *[
            ...
            for code in ANOMALY_ALGORITHMS
...
def algorithm_display_name(code: object) -> str:
    ...
    if normalized in _ANOMALY_DISPLAY_NAMES:
        return _ANOMALY_DISPLAY_NAMES[normalized]
...
def is_anomaly_tool_algorithm(code: object) -> bool:
    spec = get_tool_algorithm_spec(code)
    return bool(spec is not None and spec.family == "anomaly")
