...
class InspectionItem:
    ...
    def effective_task_group(self) -> str:
        return str(self.task_group or self.item_id or self.roi_label or self.display_name or "roi").strip() or "roi"
    ...
    def effective_model_key(self) -> str:
        if str(self.task_group or "").strip():
...
def load_inspection_items(path: str) -> List[InspectionItem]:
    if not path or not os.path.exists(path):
        return []
    try:
        raw = load_json_with_recovery(path, expected_type=list)
    except Exception:
        return []
...
def save_inspection_items(items: Iterable[InspectionItem], path: str) -> None:
    ...
    write_json_with_backup(path, payload, expected_type=list)
