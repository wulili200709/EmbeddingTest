...
from line2dup.core.recipe import Line2DupRecipe, expanded_reference_region_specs


def inspection_item_specs_from_line2dup_recipe(recipe: Line2DupRecipe | None) -> List[Dict[str, str]]:
    specs: List[Dict[str, str]] = []
    seen_labels: set[str] = set()
    for region in expanded_reference_region_specs(recipe):
        roi_label = str(region.get("roi_label", "")).strip()
        if not roi_label or roi_label in seen_labels:
            continue
        display_name = str(region.get("display_name") or roi_label).strip() or roi_label
        specs.append({
            "roi_label": roi_label,
...
def group_names_from_line2dup_recipe(recipe: Line2DupRecipe | None) -> List[str]:
    names: List[str] = []
    seen: set[str] = set()
    reference_regions = list(getattr(recipe, "reference_regions", []) or [])
    for region in reference_regions:
        if not isinstance(region, dict):
            continue
        name = str(region.get("display_name") or "").strip()
    ...
    for spec in inspection_item_specs_from_line2dup_recipe(recipe):
    ...
    *,
    ...
    def _sort_key(label: str):
        ...
        if label.startswith("roi") and label[3:].isdigit():
            return (0, int(label[3:]), 0, label)
        match = re.match(r"^roi(\d+)__(\d+)$", label)
        if match:
