...
#ifndef SHAPE_TEST_PREFIX_DEFAULT
#define SHAPE_TEST_PREFIX_DEFAULT "/home/meiqua/shape_based_matching/test"
#endif

static std::string resolve_prefix()
{
    const char* p = std::getenv("SHAPE_TEST_PREFIX");
    if (p != nullptr && p[0] != '\0')
    {
        std::string out(p);
    ...
    std::string out = SHAPE_TEST_PREFIX_DEFAULT;
...
static std::string prefix = resolve_prefix();
...
//    mode = "test";
    if(mode == "train"){
        ...
        std::vector<std::string> scene_files = {"1.jpg", "2.jpg", "3.png", "4.png"};
        ensure_dir(prefix + "case0/result_line2dup");
        for (const auto& scene_file : scene_files)
        {
            Mat test_img = imread(prefix + "case0/" + scene_file);
            if (test_img.empty())
            {
                std::cout << "skip missing scene: " << scene_file << std::endl;
                continue;
            }
            ...
            std::cout << "scene: " << scene_file << ", matches.size(): " << matches.size() << std::endl;
            size_t top5 = 5;
            ...
            std::string stem = scene_file.substr(0, scene_file.find_last_of('.'));
            std::string out_path = prefix + "case0/result_line2dup/" + stem + ".png";
...
//        cvtColor(img, img, CV_BGR2GRAY);
        ...
        ensure_dir(prefix + "case1/result_line2dup");
        cv::imwrite(prefix + "case1/result_line2dup/result.png", img);
...
void noise_test(string mode = "test"){
    ...
    if(mode == "train"){
        ...
        ensure_dir(prefix + "case2/result_line2dup");
        cv::imwrite(prefix + "case2/result_line2dup/result.png", test_img);
