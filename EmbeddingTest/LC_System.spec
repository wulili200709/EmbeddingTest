...
def _torch_polyfill_hiddenimports() -> list[str]:
    try:
        root = Path(importlib_metadata.distribution("torch").locate_file("torch/_dynamo/polyfills"))
    except Exception:
        return []
    if not root.exists():
        return []
    ...
    for path in root.glob("*.py"):
        if path.name == "__init__.py":
            continue
        hidden.append(f"torch._dynamo.polyfills.{path.stem}")
