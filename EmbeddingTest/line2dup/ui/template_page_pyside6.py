...
def _overlay_follow_result(
    ...
    *,
    ...
    if int(getattr(result, "instance_count", 1) or 1) > 1:
        text += f" instances={int(result.instance_count)}"
    transform_mode = str(getattr(result, "transform_mode", "") or "").strip()
    if transform_mode:
        text += f" mode={transform_mode}"
...
class Line2DupTemplateDialog(QtWidgets.QDialog):
    ...
    def _build_reference_tab(self) -> None:
        ...
        array_box = QtWidgets.QGroupBox("阵列")
        array_form = QtWidgets.QFormLayout(array_box)
        self.box_reference_array = array_box
        self.spin_array_count = QtWidgets.QSpinBox()
        self.spin_array_count.setRange(1, 256)
        self.spin_array_count.setValue(1)
        self.spin_array_count.valueChanged.connect(self._on_array_settings_changed)
        self.spin_array_pitch_x = QtWidgets.QDoubleSpinBox()
        self.spin_array_pitch_x.setRange(-5000.0, 5000.0)
        self.spin_array_pitch_x.setDecimals(2)
        self.spin_array_pitch_x.setValue(0.0)
        self.spin_array_pitch_x.valueChanged.connect(self._on_array_settings_changed)
        self.spin_array_pitch_y = QtWidgets.QDoubleSpinBox()
        self.spin_array_pitch_y.setRange(-5000.0, 5000.0)
        self.spin_array_pitch_y.setDecimals(2)
        self.spin_array_pitch_y.setValue(0.0)
        self.spin_array_pitch_y.valueChanged.connect(self._on_array_settings_changed)
        self.lbl_array_hint = QtWidgets.QLabel("只画当前标准片上的基准 ROI；运行时会按模板匹配到的每一片自动仿射展开。")
        self.lbl_array_hint.setWordWrap(True)
        array_form.addRow("阵列数量", self.spin_array_count)
        array_form.addRow("pitch_x", self.spin_array_pitch_x)
        array_form.addRow("pitch_y", self.spin_array_pitch_y)
        array_form.addRow("说明", self.lbl_array_hint)
        preview_row = QtWidgets.QHBoxLayout()
        self.btn_preview_array = QtWidgets.QPushButton("预览阵列")
        self.btn_preview_array.clicked.connect(self._preview_reference_array)
        self.btn_clear_array_preview = QtWidgets.QPushButton("清除预览")
        self.btn_clear_array_preview.clicked.connect(self._clear_reference_array_preview)
        preview_row.addWidget(self.btn_preview_array)
        preview_row.addWidget(self.btn_clear_array_preview)
        array_form.addRow("预览", preview_row)
        self.box_reference_array.setVisible(False)
        left.addWidget(self.box_reference_array)
    ...
    def _current_array_count(self) -> int:
        return 1

    def _current_array_pitch(self) -> Tuple[float, float]:
        return 0.0, 0.0

    def _reference_status_text(self, message: str) -> str:
        base_count = len(self._reference_regions)
        detail = f"基准ROI {base_count} 个，阵列功能已禁用。"
    ...
    def _set_reference_status(self, message: str) -> None:
        if hasattr(self, "lbl_reference_status"):
            self.lbl_reference_status.setText(self._reference_status_text(message))

    def _array_preview_validation_error(self, recipe: Line2DupRecipe) -> str:
        ...
        if not model_path or not os.path.exists(model_path):
            return "请先创建或加载模板模型。"
        if int(getattr(recipe, "array_count", 1) or 1) > 1:
            pitch_x = float(getattr(recipe, "array_pitch_x", 0.0) or 0.0)
            pitch_y = float(getattr(recipe, "array_pitch_y", 0.0) or 0.0)
            if abs(pitch_x) < 1e-6 and abs(pitch_y) < 1e-6:
                return "阵列数量大于 1 时，请先设置阵列间距 pitch_x / pitch_y。"
        return ""

    def _array_preview_palette(self) -> List[QtGui.QColor]:
        return [
            ...
            QtGui.QColor(180, 120, 255),
            QtGui.QColor(255, 80, 80),
            QtGui.QColor(100, 255, 180),
        ]

    def _array_preview_overlays(self, detector: Line2DupLikeDetector, result: FollowResult) -> List[OverlayShape]:
        palette = self._array_preview_palette()
        overlays: List[OverlayShape] = []
        matches = list(getattr(result, "matches", None) or [result.match])
        instance_count = max(1, int(getattr(result, "instance_count", len(matches)) or len(matches) or 1))
        base_region_count = max(1, len(result.regions) // instance_count) if result.regions else 1
        ...
        for instance_index, match in enumerate(matches):
            ...
            overlays.append(
                OverlayShape(
                    shape_type="polygon",
                    points=[(float(x), float(y)) for x, y in match_quad(detector, match)],
                    color=color,
                    width=2.6,
                    dash=False,
                )
        ...
        for region_index, region in enumerate(result.regions):
            instance_index = min(instance_count - 1, max(0, region_index // base_region_count))
            color = palette[instance_index % len(palette)]
            overlays.append(
                OverlayShape(
                    shape_type="polygon" if region.source_shape_type == "polygon" else "rect",
                    points=[(float(x), float(y)) for x, y in region.points] if region.source_shape_type == "polygon" else None,
                    xywh=region.bbox if region.source_shape_type != "polygon" else None,
                    color=color,
                    width=2.0,
                    dash=False,
                )
    ...
    def _preview_reference_array(self) -> None:
        recipe = self._recipe_from_controls(use_find_values=True)
        recipe.model_path = self.find_model_path or self.paths.model_path
        if not recipe.reference_image and self.image_path:
            recipe.reference_image = self.image_path
        error = self._array_preview_validation_error(recipe)
        if error:
            QtWidgets.QMessageBox.warning(self, "提示", error)
            return
        try:
            detector = self._get_detector_for_model(recipe.model_path, reuse_shared=True)
            ...
            QtWidgets.QMessageBox.warning(self, "预览失败", str(exc))
            return
        self._reference_preview_overlays = self._array_preview_overlays(detector, result)
        self._refresh_reference_canvas()
        instance_count = int(getattr(result, "instance_count", 1) or 1)
        transform_mode = str(getattr(result, "transform_mode", "affine") or "affine")
        message = (
            f"阵列预览完成：主匹配 sim={float(result.match.similarity):.2f}，"
            f"按间距生成 {instance_count} 份 ROI。"
        )
        message = f"{message} mode={transform_mode}"
        self._set_reference_status(message)

    def _clear_reference_array_preview(self) -> None:
        if not self._reference_preview_overlays:
            self._set_reference_status("当前没有阵列预览。")
            return
        self._reference_preview_overlays = []
        self._refresh_reference_canvas()
        self._set_reference_status("已清除阵列预览。")

    def _invalidate_reference_array_preview(self) -> None:
        self._reference_preview_overlays = []

    def _apply_template_param_values(
        self,
        *,
        ...
        if levels is not None:
            text = str(levels or "").strip()
            if text:
                self.edit_levels.setText(text)
        if num_features is not None and int(num_features) > 0:
            self.spin_num_features.setValue(int(num_features))
        if weak_threshold is not None:
            self.spin_weak.setValue(float(weak_threshold))
        if strong_threshold is not None:
            self.spin_strong.setValue(float(strong_threshold))
        if angle_start is not None:
            self.spin_angle_start.setValue(float(angle_start))
        if angle_end is not None:
            self.spin_angle_end.setValue(float(angle_end))
        if angle_step is not None and float(angle_step) > 0.0:
            self.spin_angle_step.setValue(float(angle_step))
        if scale_start is not None and float(scale_start) > 0.0:
            self.spin_scale_start.setValue(float(scale_start))
        if scale_end is not None and float(scale_end) > 0.0:
            self.spin_scale_end.setValue(float(scale_end))
        if scale_step is not None and float(scale_step) > 0.0:
            self.spin_scale_step.setValue(float(scale_step))

    def _apply_template_params_from_recipe(self, recipe: Line2DupRecipe) -> None:
        self._apply_template_param_values(
            levels=recipe.template_levels,
            num_features=int(recipe.template_num_features),
            weak_threshold=float(recipe.template_weak_threshold),
            strong_threshold=float(recipe.template_strong_threshold),
            angle_start=float(recipe.template_angle_start),
            angle_end=float(recipe.template_angle_end),
            angle_step=float(recipe.template_angle_step),
            scale_start=float(recipe.template_scale_start),
            scale_end=float(recipe.template_scale_end),
            scale_step=float(recipe.template_scale_step),
        )

    def _apply_template_params_from_model(self, detector: Line2DupLikeDetector, source_info: object) -> None:
        levels_text = ",".join(str(int(level)) for level in getattr(detector, "T_at_level", []) if int(level) > 0)
        pose_infos = source_info.get("pose_infos", {}) if isinstance(source_info, dict) else {}
        pose_ui = pose_infos.get("ui", {}) if isinstance(pose_infos, dict) else {}
        self._apply_template_param_values(
            levels=levels_text or None,
            num_features=int(getattr(detector, "num_features", 0) or 0),
            weak_threshold=self._safe_float(getattr(detector, "weak_threshold", None)),
            strong_threshold=self._safe_float(getattr(detector, "strong_threshold", None)),
            angle_start=self._safe_float(pose_ui.get("angle_start")) if isinstance(pose_ui, dict) else None,
            angle_end=self._safe_float(pose_ui.get("angle_end")) if isinstance(pose_ui, dict) else None,
            angle_step=self._safe_float(pose_ui.get("angle_step")) if isinstance(pose_ui, dict) else None,
            scale_start=self._safe_float(pose_ui.get("scale_start")) if isinstance(pose_ui, dict) else None,
            scale_end=self._safe_float(pose_ui.get("scale_end")) if isinstance(pose_ui, dict) else None,
            scale_step=self._safe_float(pose_ui.get("scale_step")) if isinstance(pose_ui, dict) else None,
        )

    def _update_extract_points_button_state(self) -> None:
        self.btn_extract_points.setEnabled(self.image_bgr is not None and self.template_roi is not None)

    def _has_editor_features(self) -> bool:
        level0 = self._editor_level0()
        return level0 is not None and bool(level0.features)
    ...
    def _recipe_from_controls(self, *, use_find_values: bool = False) -> Line2DupRecipe:
        ...
        return Line2DupRecipe(
            ...
            template_levels=self.edit_levels.text().strip() or "4,8",
            template_num_features=int(self.spin_num_features.value()),
            template_weak_threshold=float(self.spin_weak.value()),
            template_strong_threshold=float(self.spin_strong.value()),
            template_angle_start=float(self.spin_angle_start.value()),
            template_angle_end=float(self.spin_angle_end.value()),
            template_angle_step=float(self.spin_angle_step.value()),
            template_scale_start=float(self.spin_scale_start.value()),
            template_scale_end=float(self.spin_scale_end.value()),
            template_scale_step=float(self.spin_scale_step.value()),
            backend=backend,
            threshold=threshold,
            ...
            nms_iou=nms_iou,
            topk=self._current_array_count(),
            array_count=self._current_array_count(),
            array_pitch_x=self._current_array_pitch()[0],
            array_pitch_y=self._current_array_pitch()[1],
            crop_stride=0,
    ...
    def _load_recipe(self) -> None:
        ...
        self._apply_template_params_from_recipe(recipe)
        ...
        with QtCore.QSignalBlocker(self.spin_array_count):
            self.spin_array_count.setValue(1)
        with QtCore.QSignalBlocker(self.spin_array_pitch_x):
            self.spin_array_pitch_x.setValue(0.0)
        with QtCore.QSignalBlocker(self.spin_array_pitch_y):
            self.spin_array_pitch_y.setValue(0.0)
    ...
    def _load_existing_model(self, *, silent: bool) -> None:
        ...
        self._apply_template_params_from_model(detector, source_info)
    ...
    def _on_array_settings_changed(self, _value) -> None:
        ...
        self._set_reference_status("已更新阵列参数。")
    ...
    def _run_find_for_item(self, item: QtWidgets.QListWidgetItem) -> None:
        ...
        if not recipe.reference_image or not os.path.exists(recipe.reference_image):
            ...
            return
        array_error = self._array_preview_validation_error(recipe)
        if array_error:
            self._set_find_item_error(item, array_error)
            return
        ...
        if int(getattr(result, "instance_count", 1) or 1) > 1:
            msg += f" instances={int(result.instance_count)}"
        transform_mode = str(getattr(result, "transform_mode", "") or "").strip()
        if transform_mode:
            msg += f" mode={transform_mode}"
