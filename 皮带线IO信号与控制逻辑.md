...
| DI6 | 10 | Detection Sensor3 | `end_test_sensor` | 皮带末端测试堵料传感器 | **高有效：0→1触发** |
...
| DI6 末端测试传感器 | 0、灯灭 | 1、灯亮 | 0→1表示业务有效；持续高电平按堵料逻辑计时 |
...
- `SafetyInterlockController`：管理 DI5、DI9、DI10和安全许可；
...
- 报警确认/复位；
- FIFO 在途物料数量；
- FIFO 状态灯：DI0触发立即新增黄色灯，检测OK变绿色、NG/非OK变红色，DI1消费对应物料后移除；
- 待检测数量；
- GOOD 待通过数量；
- NG 待剔除数量；
- DI0～DI10实时业务状态；
...
运行界面不重复显示启动、停止按钮，正常启动和停止由实体 DI2、DI3 操作。同一个清线按钮在停止状态显示“一键清线”，执行中显示“清线中…”，安全或人工停止导致清线暂停后显示“继续清线”，不再单独显示第二个继续按钮。“报警复位”仅在产线处于 `FAULT`、IO 正常、DI5 有效且门联锁满足时允许点击，复位后保持停止且不得自动启动。
...
10. 已确认检测 ERROR、模板匹配失败及其他非 OK 结果统一按 NG 吹气，不停机报警。
