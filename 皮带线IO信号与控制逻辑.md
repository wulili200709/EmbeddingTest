...
本节是现场重新确认后的目标逻辑：DI6是专用堵料传感器；DI7、DI8既要确认逐件出料，也要通过持续有效时间检测出口堵料。当前代码只实现了DI7、DI8持续有效堵料监控，尚未实现逐件出口等待、未到达超时和走错出口判断，后续代码必须按本节改造。
...
DI8 当前为有效（废料口正有物料）**不作为清线启动否决条件**，因为清线本身就是要让皮带和持续吹气把物料带离废料口。进入清线后，DI8 仍按“皮带实际运行时间”累计堵料计时：若持续有效超过目标参数 `waste_outlet_blocked_timeout_s`（当前代码参数名为 `waste_jam_timeout_s`），才锁存 `JAM_DETECTED` 并立即停止。若清线前已经锁存堵料故障，则必须先清除 DI8 现场堵料并确认报警，不能绕过故障直接动作。
...
当前代码中的 `end_test_jam_timeout_s`、`good_jam_timeout_s`、`waste_jam_timeout_s` 仅实现持续有效堵料计时。后续代码实现出口确认时，应将堵料参数统一为上述明确名称，同时保留旧配置名称的读取兼容。
...
DI8同理。但如果两个产品在DI0、DI1、DI7/DI8均形成一个长脉冲，单靠这些传感器仍可能把两件当成一件，必须同时实施：

- 机械分料，保证一件产品对应一个完整传感器脉冲；
- DI0最大持续有效时间和最小无料间隙监控；
- 视觉画面产品数量校验：0件、1件、多件；
- 视野内出现多件时报告 `MULTIPLE_PRODUCTS_IN_FOV`，停止并要求清线，不能简单按一个NG处理。
...
- `OutletConfirmationTracker`（待新增）：管理GOOD→DI7、NG→DI8的逐件等待、合法时间窗口、出口匹配和完成确认；
...
以上为现场定义更新后的目标参数结构。当前 `config/defaults/conveyor_control.json` 和 `ConveyorConfig` 尚未加入出口到达窗口，堵料参数也仍使用 `end_test_jam_timeout_s`、`good_jam_timeout_s`、`waste_jam_timeout_s`；在对应代码改造完成前，新参数不会生效。实施时必须同步更新代码、默认JSON和测试，并兼容读取旧名称（例如 `reject_delay_ms`、`fifo_max_items`以及旧堵料参数名），避免文档、配置与代码再次错位。
